...
def index(request):
		...
		response.update({'ad':Ad.objects.all().order_by('?')[:1]})
...
def user_edit_answer(request):
	...
	if 'answer_id' in request.POST and request.POST['answer_id']:
		...
		return render_to_response('user.html', response)
...
def profile_pic(request):
	...
	response.update({'user':request.user})
	response.update({'next':request.get_full_path()})
	response.update({'profile_pic':True})

	if 'profile' in request.FILES and request.FILES['profile']:
		uploaded_file = request.FILES['profile']
		try:
			f = Image.open(uploaded_file)
		except:
			response.update({'not_image':True}) 
			return render_to_response('user.html', response)
		...
		if os.path.exists(filename):
			return HttpResponse('Cannot upload your image as a file with same name exists. Please rename the file and upload again.')
		...
		try:
			...
			bucket = conn.create_bucket('uploads.siqa.com')
			k = Key(bucket)
			k.key = key_name
			...
			img_url = 'https://s3.amazonaws.com/uploads.siqa.com/' + key_name
			
			try:
				profile = Profile.objects.get(user=request.user)
			except:
				Profile.objects.create(user=request.user, profile_pic=img_url)
			else:
...
def question_unanswered(request):
	...
	questions = Question.objects.filter(answer=None).distinct()
	response.update({'count':questions.count()})
